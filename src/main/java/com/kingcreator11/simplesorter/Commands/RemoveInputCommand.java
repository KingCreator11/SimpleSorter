...
/**
 * The remove input sub command
 */
public class RemoveInputCommand extends SubCommand {

	/**
	 * Creates a new remove input command instance
	 * @param plugin
	 */
	public RemoveInputCommand(SimpleSorter plugin) {
	...
	protected void executeCommand(CommandSender sender, String argument) {
		...
		String dbresult = this.plugin.inputManager.removeInput(block.getLocation(), player.getUniqueId().toString());
		if (dbresult == null) {
