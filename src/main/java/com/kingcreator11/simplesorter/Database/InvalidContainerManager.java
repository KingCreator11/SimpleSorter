...
public class InvalidContainerManager extends SimpleSorterBase {
	 ...
	 */
	public String addInvalidContainer(Location location, String sorter, String playerUUID) {
		if (this.plugin.sorterManager.doesContainerExist(location))
			return "A sorter is already using a container at that location";

		try {
			String sql = "INSERT INTO `invaliditemchests` (x, y, z, world, sorterId) VALUES (?, ?, ?, ?, ?)";
			...
			this.plugin.getLogger().warning("Unable to add invalid at " + location + " for sorter " + sorter
	 ...
	 */
	public String removeInvalidContainer(Location location, String playerUUID) {
	...
	public DBContainer getContainer(Location location) {
		try {
			String sql = "SELECT * FROM `invaliditemchests` WHERE x = ? AND y = ? AND z = ? AND world = ?";
			PreparedStatement statement = this.plugin.dbManager.db.prepareStatement(sql);
			statement.setInt(1, (int) location.getX());
			statement.setInt(2, (int) location.getY());
			statement.setInt(3, (int) location.getZ());
